...
plt.subplot(2, 2, 1)
for i in names:
    data = pd.read_csv(f'runs/exp/{i}/results.csv')
...
for i in names:
    data = pd.read_csv(f'runs/exp/{i}/results.csv')
    data['exp/box_loss'] = data['exp/box_loss'].astype(np.float32).replace(np.inf, np.nan)
    data['exp/box_loss'] = data['exp/box_loss'].fillna(data['exp/box_loss'].interpolate())
    plt.plot(data['exp/box_loss'], label=i)
plt.xlabel('epoch')
plt.title('exp/box_loss')
plt.legend()

plt.subplot(2, 3, 2)
for i in names:
    data = pd.read_csv(f'runs/exp/{i}/results.csv')
    data['exp/dfl_loss'] = data['exp/dfl_loss'].astype(np.float32).replace(np.inf, np.nan)
    data['exp/dfl_loss'] = data['exp/dfl_loss'].fillna(data['exp/dfl_loss'].interpolate())
    plt.plot(data['exp/dfl_loss'], label=i)
plt.xlabel('epoch')
plt.title('exp/dfl_loss')
plt.legend()

plt.subplot(2, 3, 3)
for i in names:
    data = pd.read_csv(f'runs/exp/{i}/results.csv')
    data['exp/cls_loss'] = data['exp/cls_loss'].astype(np.float32).replace(np.inf, np.nan)
    data['exp/cls_loss'] = data['exp/cls_loss'].fillna(data['exp/cls_loss'].interpolate())
    plt.plot(data['exp/cls_loss'], label=i)
plt.xlabel('epoch')
plt.title('exp/cls_loss')
plt.legend()
